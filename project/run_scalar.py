...
class Linear(minitorch.Module):
    ...
    def forward(self, inputs):
        outputs = []
        for j in range(len(self.bias)):  # Iterate over the output neurons
            output = self.bias[j].value  # Start with the bias for the j-th output
        for i in range(len(inputs)):  # Iterate over the inputs
            output += (
                self.weights[i][j].value * inputs[i]
            )  # Multiply input by its weight and add to the output
